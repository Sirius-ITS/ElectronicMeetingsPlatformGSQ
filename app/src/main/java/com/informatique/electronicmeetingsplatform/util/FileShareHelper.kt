...
object FileShareHelper {
    ...
    fun getShareableUri(context: Context, uri: Uri, fileName: String?): Uri? {
        return try {
            Log.d("FileShareHelper", "Getting shareable URI for: $uri, fileName: $fileName")
            ...
            when (uri.scheme) {
                "content" -> {
                    ...
                    Log.d("FileShareHelper", "Content URI detected, copying to cache for sharing")
                ...
                "file" -> {
                    ...
                    if (!file.exists()) {
                        Log.e("FileShareHelper", "File does not exist: ${file.absolutePath}")
                        return null
                    }
                    ...
                    try {
                        ...
                        Log.d("FileShareHelper", "Converted file:// to FileProvider URI: $providerUri")
                        providerUri
                    } catch (e: IllegalArgumentException) {
                        Log.w("FileShareHelper", "File is not in FileProvider paths, copying to cache")
                    ...
                    Log.w("FileShareHelper", "Unknown URI scheme: ${uri.scheme}, attempting to copy")
                    copyToCache(context, uri, fileName)
                }
            }

        } catch (e: Exception) {
            Log.e("FileShareHelper", "Error creating shareable URI: ${e.message}", e)
            null
        }
    }
    ...
    private fun copyToCache(context: Context, uri: Uri, fileName: String?): Uri? {
        return try {
            ...
            if (!sharedDir.exists()) {
                ...
                Log.d("FileShareHelper", "Shared directory created: $created")
            }
            ...
            Log.d("FileShareHelper", "Using file name: $actualFileName")
            ...
            if (destFile.exists() && (System.currentTimeMillis() - destFile.lastModified() < 5 * 60 * 1000)) {
                Log.d("FileShareHelper", "Reusing existing cached file: ${destFile.absolutePath}")
            } else {
                ...
                try {
                    ...
                    Log.d("FileShareHelper", "Successfully took persistent URI permission")
                } catch (e: SecurityException) {
                    Log.w("FileShareHelper", "Could not take persistent permission, proceeding anyway")
                }
                ...
                context.contentResolver.openInputStream(uri)?.use { input ->
                    ...
                    Log.e("FileShareHelper", "Failed to open input stream from URI")
                    return null
                }

                Log.d("FileShareHelper", "Copied $bytesCopied bytes to ${destFile.absolutePath}")
            }
            ...
            Log.d("FileShareHelper", "Generated FileProvider URI: $providerUri")
            providerUri

        } catch (e: Exception) {
            Log.e("FileShareHelper", "Error copying to cache: ${e.message}", e)
            e.printStackTrace()
            null
        }
    }
        ...
        try {
            ...
            directory.listFiles()?.forEach { file ->
                if (currentTime - file.lastModified() > oneHourInMillis) {
                    file.delete()
                    Log.d("FileShareHelper", "Deleted old file: ${file.name}")
                }
            }
        } catch (e: Exception) {
            Log.e("FileShareHelper", "Error cleaning up old files: ${e.message}")
        }
    }
    ...
    private fun getFileNameFromUri(context: Context, uri: Uri): String? {
        return try {
            ...
            Log.e("FileShareHelper", "Error getting file name from URI: ${e.message}")
            uri.lastPathSegment
        }
    }
}
